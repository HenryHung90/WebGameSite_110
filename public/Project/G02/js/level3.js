...
const Level3 = {
    ...
    create: function () {
        ...
        this.Level = this.add.text(25, ch - 445, "Level 3", { fontSize: '22px', fill: '#000000' })

        let gametime = setInterval(() => {
            this.TimeStep--;
            this.timeText.setText(`TIME: ${this.TimeStep}`);
            if (this.TimeStep < 20 && this.TimeStep > 15) {
                this.bgSpeed = 1.8;
            } else if (this.TimeStep < 15   && this.TimeStep > 0) {
                this.bgSpeed = 2.5;
                ...
                let playAgainBtn = this.add.image(cw / 2, ch / 2 + 80, 'playAgainBtn');
        ...
        const masPos = [
            ...
            { name: 'mounster1', x: cw + 200, y: ch / 2 - 30, w: 150, h: 144 },
        ...
        for (let i = 0; i < 10; i++) {
            ...
            this['rock' + i] = this.physics.add.sprite(masPos[BoolIdx].x, masPos[BoolIdx].y, masPos[BoolIdx].name);
            this['rockB' + i] = this.physics.add.sprite(masPos[BoolIdx2].x, masPos[BoolIdx2].y, masPos[BoolIdx2].name);
    ...
    update: function () {
        ...
        for (let i = 0; i < this.monsterArr.length; i++) {
            this.monsterArr[i].y += Math.sin(this.TimeStep * 0.5) * 2; 
            this.monsterArr2[i].y += Math.sin(this.TimeStep * 0.5) * 2; 
        }
