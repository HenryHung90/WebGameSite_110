...
class scene5 extends Phaser.Scene {
    ...
    preload() {
        this.load.image('bg5', 'assets/png/Scene5.jpg');
        ...
        this.load.spritesheet('cat', 'assets/png/cat.png', { frameWidth: 200, frameHeight: 600 });
        ...
        this.load.audio('bgMusic5', 'assets/audio/bgMusic5.mp3');
    }
    create(data) {
        if (data.Live) {
            live = data.Live;
        ...
        bgMusic = this.sound.add('bgMusic5',
            {
                volume: 1,
                loop: true
            }
        ...
        for (let x = 0; x < 6; x++) {
            this.add.image(1380 * x, 0, 'bg5').setOrigin(0);
        }
        ...
        platforms.create(700, 400, 'short-platform').setOrigin(0).refreshBody();
        platforms.create(950, 200, 'long-platform').setOrigin(0).refreshBody();
        platforms.create(1200, 400, 'long-platform').setOrigin(0).refreshBody();
        platforms.create(1800, 400, 'long-platform').setOrigin(0).refreshBody();
        platforms.create(2050, 200, 'long-platform').setOrigin(0).refreshBody();
        platforms.create(2450, 200, 'short-platform').setOrigin(0).refreshBody();
        platforms.create(3000, 400, 'long-platform').setOrigin(0).refreshBody();
        platforms.create(3400, 200, 'short-platform').setOrigin(0).refreshBody();
        platforms.create(3800, 400, 'short-platform').setOrigin(0).refreshBody();
        platforms.create(4000, 200, 'long-platform').setOrigin(0).refreshBody();

        ice_creams.add(this.add.existing(new Ice_cream(this, 950, 500, 500, 1500, Phaser.Math.Between(110, 140), true, 1)).setOrigin(0));
        ice_creams.add(this.add.existing(new Ice_cream(this, 1625, 500, 1500, 1900, Phaser.Math.Between(110, 140), false, 2)).setOrigin(0));
        ice_creams.add(this.add.existing(new Ice_cream(this, 2500, 100, 2480, 2550, 80, true, 3)).setOrigin(0));
        ice_creams.add(this.add.existing(new Ice_cream(this, 2625, 500, 2200, 2900, Phaser.Math.Between(110, 140), true, 3)).setOrigin(0));
        ice_creams.add(this.add.existing(new Ice_cream(this, 2825, 500, 2700, 2900, Phaser.Math.Between(110, 140), true, 1)).setOrigin(0));
        ice_creams.add(this.add.existing(new Ice_cream(this, 3100, 300, 2700, 2975, 80, false, 2)).setOrigin(0));
        ice_creams.add(this.add.existing(new Ice_cream(this, 3800, 500, 3000, 3650, Phaser.Math.Between(110, 140), true, 1)).setOrigin(0));
        ice_creams.add(this.add.existing(new Ice_cream(this, 4000, 500, 3800, 4350, Phaser.Math.Between(110, 140), true, 3)).setOrigin(0));

        ices.add(this.add.existing(new Ice(this, 1200, 500)).setOrigin(0));
        ices.add(this.add.existing(new Ice(this, 2100, 500)).setOrigin(0));
        ices.add(this.add.existing(new Ice(this, 2500, 350)).setOrigin(0));
        ices.add(this.add.existing(new Ice(this, 3400, 100)).setOrigin(0));
        ices.add(this.add.existing(new Ice(this, 4050, 100)).setOrigin(0));

        let cat = this.add.existing(new Cat(this, 0, 0)).setOrigin(0).setScrollFactor(0);
        cat.anims.play('move');
        ...
        this.physics.add.collider(player, cat, this.Lose.bind(this));
        ...
        this.physics.add.collider(ices, platforms);
    ...
    update() {
        let keys = this.input.keyboard.addKeys('W,A,S,D,SPACE,E,R,Z,FIVE');
        if (keys.Z.isDown && keys.FIVE.isDown) {
            this.Skip();
        }
    ...
    setUI() {
        ...
        levelText = this.add.text(800 / 2 - 50, 0, '關卡:3-1', {
            fontSize: '25px 微軟正黑體',
            color: 'black',
            padding: 10,
