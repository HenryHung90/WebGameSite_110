import Goose from '../sprites/goose.js'
...
class scene2 extends Phaser.Scene {
    ...
    preload() {
        this.load.image('bg2', 'assets/png/Scene2.png');
        ...
        this.load.spritesheet('sun', 'assets/png/sun-spritesheet.png', { frameWidth: 100, frameHeight: 100 });
        this.load.spritesheet('goose', 'assets/spritesheet/goose.png', { frameWidth: 100, frameHeight: 100 });
    ...
    create(data) {
        ...
        this.physics.world.setBounds(0, 0, 800 * 5, 600);
        ...
        for (let x = 0; x < 4; x++) {
            this.add.image(1094 * x, 0, 'bg2').setOrigin(0);
        }

        let gooses = this.physics.add.group();
        let suns = this.physics.add.staticGroup();
        platforms = this.physics.add.staticGroup();
        let door = this.physics.add.image(3742, 400, 'door').setOrigin(0);
        ...
        gooses.add(this.add.existing(new Goose(this, 700, 500, 500, 1100, Phaser.Math.Between(100, 120), false)).setOrigin(0));
        gooses.add(this.add.existing(new Goose(this, 1300, 500, 1000, 1600, Phaser.Math.Between(100, 120), true)).setOrigin(0));
        gooses.add(this.add.existing(new Goose(this, 2000, 500, 1600, 2100, Phaser.Math.Between(100, 120), false)).setOrigin(0));
        gooses.add(this.add.existing(new Goose(this, 2100, 300, 2000, 2450, Phaser.Math.Between(100, 120), true)).setOrigin(0));
        gooses.add(this.add.existing(new Goose(this, 2550, 500, 2500, 2900, Phaser.Math.Between(100, 120), false)).setOrigin(0));
        gooses.add(this.add.existing(new Goose(this, 3300, 500, 3000, 3550, Phaser.Math.Between(100, 120), false)).setOrigin(0));

        platforms.create(700, 400, 'short-platform').setOrigin(0).refreshBody();
        platforms.create(900, 400, 'short-platform').setOrigin(0).refreshBody();
        platforms.create(1100, 200, 'short-platform').setOrigin(0).refreshBody();
        platforms.create(1550, 200, 'long-platform').setOrigin(0).refreshBody();
        platforms.create(2100, 400, 'short-platform').setOrigin(0).refreshBody();
        platforms.create(2300, 400, 'short-platform').setOrigin(0).refreshBody();
        platforms.create(2700, 200, 'short-platform').setOrigin(0).refreshBody();

        suns.add(this.add.existing(new Sun(this, 800, 300)).setOrigin(0));
        suns.add(this.add.existing(new Sun(this, 1650, 100)).setOrigin(0));
        suns.add(this.add.existing(new Sun(this, 3100, 100)).setOrigin(0));
        suns.add(this.add.existing(new Sun(this, 3400, 300)).setOrigin(0));

        this.physics.add.collider(player, door, this.Clear.bind(this));
        this.physics.add.collider(player, suns, this.Lose.bind(this));
        this.physics.add.collider(player, gooses, this.Lose.bind(this));

        gooses.getChildren().forEach((child) => {
            child.setImmovable();
            child.setCollideWorldBounds(true);
            child.anims.play('goose_move');
            if (child.right) {
        ...
        suns.getChildren().forEach((child) => {
            child.setImmovable();
    ...
    update() {
        let keys = this.input.keyboard.addKeys('W,A,S,D,SPACE,E,R,Z,TWO');
        if (keys.Z.isDown && keys.TWO.isDown) {
            this.Skip();
        }
    ...
    setUI() {
        ...
        levelText = this.add.text(800 / 2 - 50, 0, '關卡:1-2', {
            fontSize: '25px 微軟正黑體',
            color: 'black',
            padding: 10,
        }).setDepth(5).setScrollFactor(0);

        pausebtn = this.add.image(750, 50, 'pause').setDepth(2).setScrollFactor(0).setScale(0.12);
